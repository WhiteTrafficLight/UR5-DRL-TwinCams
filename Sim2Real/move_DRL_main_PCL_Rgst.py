...
from OverlapPredator.scripts.module import PointCloudMatcher
from OverlapPredator.lib.benchmark_utils import to_o3d_pcd
import pandas as pd
...
class listener_node_one:
    def __init__(self, action_rate, control_rate, num_voxels, point_cloud_static):
        ...
        self.pose_estimate_config = 'OverlapPredator/configs/test/indoor.yaml'
        self.pose_estimate_model = 'OverlapPredator/weights/indoor.pth'
        self.pcl_matcher = PointCloudMatcher(self.pose_estimate_config,self.pose_estimate_model)
        ...
        sleep(1)
        print("[Listener] Started callback for raw pointcloud2 data")
        rospy.Subscriber("/camera2/depth/points", PointCloud2, self.cbGetPointcloud2)
        sleep(1)
    ...
    def cbGetPointcloud(self, data):
        ...
        self.colors2 = color_floats
        self.points_raw = points
        self.data_set_guard = False

    def cbGetPointCloud2(self,data):
        np_data = ros_numpy.numpify(data)
        points = np.ones((np_data.shape[0]*np_data.shape[1], 4))
        if len(points) == 0:
            print("[cbGetPointcloud] No point cloud data received, check the camera and its ROS program!")
    ...
    def PointcloudToVoxel(self):
        if self.joints is not None and len(self.points_raw) != 0 and len(self.points_raw2) != 0:
            while self.data_set_guard:
                sleep(0.0001)
            
            points = self.points_raw
            colors = self.colors
            points2 = self.points_raw2
            colors2 = self.colors2
            
            if self.use_gpu:
                points = torch.from_numpy(points).to('cuda')
                colors = torch.from_numpy(colors).to('cuda')
                points2 = torch.from_numpy(points2).to('cuda')
                colors2 = torch.from_numpy(colors2).to('cuda')

            # Estimate the transformation from points2 to points
            trfm = self.pcl_matcher.estimate_transform(points2.cpu().numpy(), points.cpu().numpy())
            points2_transformed = np.dot(trfm[:3, :3], points2.cpu().numpy().T).T + trfm[:3, 3]

            if self.use_gpu:
                points2_transformed = torch.from_numpy(points2_transformed).to('cuda')
            
            # Combine both point clouds
            points = torch.cat((points, points2_transformed), dim=0) if self.use_gpu else np.concatenate((points, points2_transformed), axis=0)
            colors = torch.cat((colors, colors2), dim=0) if self.use_gpu else np.concatenate((colors, colors2), axis=0)

            # Filter points within the bounds for both point clouds
            lower_mask = (points >= self.points_lower_bound_camera).all(axis=1)
            upper_mask = (points <= self.points_upper_bound_camera).all(axis=1)
            in_boundary_mask = torch.logical_and(lower_mask, upper_mask) if self.use_gpu else np.logical_and(lower_mask, upper_mask)
            points = points[in_boundary_mask]
            colors = colors[in_boundary_mask]

            # Transform points to PyBullet coordinate system
            if self.use_gpu:
                points = torch.matmul(self.camera_transform_gpu, points.T).T[:, :3]
            else:
                points = np.dot(self.camera_transform_to_pyb_origin, points.T).T[:, :3]
            ...
            if not self.camera_calibration:
                ...
                mask_offset1 = torch.from_numpy(ee_pos + np.array([0.1, -0.1, 0.2])).to('cuda') if self.use_gpu else ee_pos + np.array([0.1, -0.1, 0.2])
                mask_offset2 = torch.from_numpy(ee_pos + np.array([0, 0, 0.15])).to('cuda') if self.use_gpu else ee_pos + np.array([0, 0, 0.15])
                mask_left = self.delete_points_by_circle_center(points, mask_offset1)
                mask_above = self.delete_points_by_circle_center(points, mask_offset2)
                not_close_mask = torch.logical_and(mask_left, mask_above) if self.use_gpu else np.logical_and(mask_left, mask_above)
            ...
            voxel_centers = voxel_centers * self.voxel_size + self.points_lower_bound[:3]
            ...
            pyb_u.set_base_pos_and_ori(self.probe_voxel.object_id, self.pos_nowhere, np.array([0, 0, 0, 1]))
