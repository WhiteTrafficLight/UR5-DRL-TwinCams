...
class listener_node_one:
    def __init__(self, action_rate, control_rate, num_voxels, point_cloud_static):
        ...
        # Initialize transformation for the second camera
        self.second_camera_transform_to_pyb_origin = np.eye(4)
        second_camera_rpy = np.array(self.config['camera2_transform_to_pyb_origin']['rpy'])
        second_camera_matrix = (R.from_euler('xyz', second_camera_rpy, degrees=True)).as_matrix()
        self.second_camera_transform_to_pyb_origin[:3, :3] = second_camera_matrix
        self.second_camera_transform_to_pyb_origin[:3, 3] = np.array(self.config['camera2_transform_to_pyb_origin']['xyz'])

        if self.use_gpu:
            self.second_camera_transform_gpu = torch.from_numpy(self.second_camera_transform_to_pyb_origin).to('cuda')    
    ...
    def cbControl(self, event):  
        ...
        if self.goal is None and self.joints is not None:
            ...
            if self.drl_success and self.sim_step == self.real_step: 
                ...
                self.drl_success = False
                self.inference_done = False
                print("[cbControl] Goal reached, Task done successfully!")
    ...
    def cbGetPointcloud2(self, data):
        ...
        points = np.ones((np_data.shape[0]*np_data.shape[1], 4))
        if len(points) == 0:
            print("[cbGetPointcloud2] No point cloud data received from second camera, check the camera and its ROS program!")
            return    
        points[:, 0] = np_data['x'].flatten()
        points[:, 1] = np_data['y'].flatten()
        points[:, 2] = np_data['z'].flatten()
    ...
    def PointcloudToVoxel(self):
        if self.joints is not None and len(self.points_raw) != 0:
            ...
            if self.points_raw2 is not None and len(self.points_raw2) != 0:
                ...
                lower_mask2 = (points2 >= self.points_lower_bound_camera).all(axis=1)
                upper_mask2 = (points2 <= self.points_upper_bound_camera).all(axis=1)
                if self.use_gpu:
                    in_boundary_mask2 = torch.logical_and(lower_mask2, upper_mask2)
                else:
                    in_boundary_mask2 = np.logical_and(lower_mask2, upper_mask2)
                points2 = points2[in_boundary_mask2]
                colors2 = colors2[in_boundary_mask2]

                if not self.use_gpu:
                    points2 = np.dot(self.second_camera_transform_to_pyb_origin, points2.T).T.reshape(-1, 4)
                else:
                    points2 = torch.matmul(self.second_camera_transform_gpu, points2.T).T.reshape(-1, 4)

                points2 = points2[:, :3]

                if not self.camera_calibration:
                    mask_left2 = self.delete_points_by_circle_center(points2, mask_offset1)
                    mask_above2 = self.delete_points_by_circle_center(points2, mask_offset2)
                    if self.use_gpu:
                        not_close_mask2 = torch.logical_and(mask_left2, mask_above2)
                    else:
                        not_close_mask2 = np.logical_and(mask_left2, mask_above2)
                    points2 = points2[not_close_mask2]
                    colors2 = colors2[not_close_mask2]

                if len(points2) != 0:
                    if self.use_gpu:
                        points2 = points2.cpu().numpy()
                        colors2 = colors2.cpu().numpy()
